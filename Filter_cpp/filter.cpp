...
//640ピクセル,3ライン
  ...
  int *out_img_buf = (int *)malloc(3*(640-2)*sizeof(int));
    ...
    LOAD_IMG1: for(int x = 0; x < 640; x++){
      in_img_buf_r[640*0+x] = stream_in[x*3];
      in_img_buf_g[640*0+x] = stream_in[x*3+1];
      in_img_buf_b[640*0+x] = stream_in[x*3+2];
    }

    LOAD_IMG2: for(int x = 0; x < 640; x++){
     in_img_buf_r[640*1+x] = stream_in[x*3+640];
     in_img_buf_g[640*1+x] = stream_in[x*3+1+640];
     in_img_buf_b[640*1+x] = stream_in[x*3+2+640];
    }
#endif
...
#else
      for(int x = 0; x < 640; x++){
        in_img_buf_r[640*(y%3)+x] = stream_in[y*640+x*3];
        in_img_buf_g[640*(y%3)+x] = stream_in[y*640+x*3+1];
        in_img_buf_b[640*(y%3)+x] = stream_in[y*640+x*3+2];
      }
#endif
